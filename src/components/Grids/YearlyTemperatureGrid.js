...
let showing = 1;
class YearlyTemperatureGrid extends Component {
  ...
  constructor(props) {
    ...
    this.loadTempYearlyAverage = this.loadTempYearlyAverage.bind(this)
  ...
  async componentDidMount() {
    ...
    if (this.state.showing === 1)
      await this.loadTempYearlyAverage()
  ...
  async loadTempYearlyAverage() {

    let result = await API.loadTemperatureYearlyAvg()
    console.log(result)
    if (result === null ) {
      ...
      xl={(this.props.sizes.xl>=12)?12:this.props.sizes.xl}  >
            {this.state.data ?
              ...
              this.state.data === 'ERROR' ?
                ...
                Charts.renderTemperatureYearlyAvgBarGraph(this.state.data,this.props.sizes.height) :
  ...
  render() {
    ...
    if (this.state.showing === 1) {
      return this.renderTempYearlyAvgGrid()
    }
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter((YearlyTemperatureGrid)));