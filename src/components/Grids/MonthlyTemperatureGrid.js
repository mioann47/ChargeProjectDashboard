...
let showing = 1;
class MonthlyTemperatureGrid extends Component {
  ...
  constructor(props) {
    ...
    this.loadTempMonthlyAverage = this.loadTempMonthlyAverage.bind(this)
  ...
  async componentDidMount() {
    ...
    if (this.state.showing === 1)
      await this.loadTempMonthlyAverage(2017)
  ...
  async loadTempMonthlyAverage(year) {

    let result = await API.loadTemperatureMonthlyAvg(year)
    console.log(result)
    if (result === null ) {
  ...
  async handleLoadData(year) {
    await this.loadTempMonthlyAverage(year)
  }
  ...
  async handleApply() {
      ...
      await this.loadTempMonthlyAverage()
      ...
      xl={(this.props.sizes.xl >= 12) ? 12 : this.props.sizes.xl} >
            {this.state.data ?
              ...
              this.state.data === 'ERROR' ?
                ...
                Charts.renderTemperatureMonthlyAvgBarGraph(this.state.data,this.props.sizes.height) :
  ...
  render() {
    ...
    if (this.state.showing === 1) {
      return this.renderTempMonthlyAvgGrid()
    }
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter((MonthlyTemperatureGrid)));