...
let showing = 1;
class MonthlyHumidityGrid extends Component {
  ...
  constructor(props) {
    ...
    this.loadHumidityMonthlyAverage = this.loadHumidityMonthlyAverage.bind(this)
  ...
  async componentDidMount() {
    ...
    if (this.state.showing === 1)
      await this.loadHumidityMonthlyAverage(2017)
  ...
  async loadHumidityMonthlyAverage(year) {

    let result = await API.loadHumidityMonthlyAvg(year)
    console.log(result)
    if (result === null ) {
  ...
  async handleLoadData(year) {
    await this.loadHumidityMonthlyAverage(year)
  }

  async handleApply() {

      await this.loadHumidityMonthlyAverage()
          ...
          xl={(this.props.sizes.xl>=12)?12:this.props.sizes.xl} >
            {this.state.data ?
              ...
              this.state.data === 'ERROR' ?
                ...
                Charts.renderHumidityMonthlyAvgBarGraph(this.state.data,this.props.sizes.height) :
  ...
  render() {
    ...
    if (this.state.showing === 1) {
      return this.renderHumidityMonthlyAvgGrid()
    }
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter((MonthlyHumidityGrid)));