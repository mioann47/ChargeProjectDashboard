...
let showing = 1;
class MonthlyIrradianceGrid extends Component {
  ...
  constructor(props) {
    ...
    this.loadIrradianceMonthlyAverage = this.loadIrradianceMonthlyAverage.bind(this)
  ...
  async componentDidMount() {
    ...
    if (this.state.showing === 1)
      await this.loadIrradianceMonthlyAverage(2017)
  ...
  async loadIrradianceMonthlyAverage(year) {

    let result = await API.loadIrradianceMonthlyAvg(year)
    console.log(result)
    if (result === null ) {
  ...
  async handleLoadData(year) {
    await this.loadIrradianceMonthlyAverage(year)
  }
  ...
  async handleApply() {
      ...
      await this.loadIrradianceMonthlyAverage()
  ...
  renderIrradianceMonthlyAvgGrid() {
        ...
        xl={(this.props.sizes.xl>=12)?12:this.props.sizes.xl}  >
            {this.state.data ?
              ...
              this.state.data === 'ERROR' ?
                ...
                Charts.renderIrradianceMonthlyAvgBarGraph(this.state.data,this.props.sizes.height) :
  ...
  render() {
    ...
    if (this.state.showing === 1) {
      return this.renderIrradianceMonthlyAvgGrid()
    }
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter((MonthlyIrradianceGrid)));